...
Figure 1. Jumper connections schematic

![Schematic](./images/SchematicSnapshot.png)

## Input voltages

Input voltage supply to CYW920706WCDEVAL board:
1) J3 goes to BT_VDDIO and should be 3.3V
2) J9 goes to VBAT (input of internal 1.2V LDO) and should be 3.3V
3) J10 goes to VIN_BTLDO2P5 (input of internal 2.5V LDO) and should be 3.3V
...
Therefore, you should use a multimeter that provides the option to set the ”aperture” of the measurement. The aperture is the period ”T” during which the multimeter measures the instantaneous currents, integrates them, and then displays the average current for the period ”T”.

Please remove jumper J3 and connect the multimeter to measure the current in normal mode and low power mode.

Please remove jumper J9 and connect the multimeter to measure the current in normal mode and low power mode.

Please remove jumper J10 and connect the multimeter to measure the current in normal mode and low power mode.

The total chip current consumption is the current consumption of J3 + current consumption of J9 + current consumption of J10 for CYW920706WCDWVAL.

The following table shows the example of CYW920706WCDEVAL current consumption in different modes

| State                                   | J3 Pin                              | J9 Pin   | J10       | Total (J3+J9+J10)  |
|-----------------------------------------|-------------------------------------|----------|-----------|--------------------|
| Normal Mode (D9 is on D10 is blinking)  | ~94 uA (D10 on) or ~26uA (D10 off)  | ~6.7 mA  | ~8.8 uA   | ~6.803 mA          |
| Low Power Mode (D9 and D10 are off)     | ~0.8 uA                             | ~0.1 uA  | ~0.2 uA   | ~1.1 uA            |


Note: The values on J3 will fluctuate due to D10 toggling. If desired, that can be eliminated by editing low_power_20706.c to disable the 2 calls to wiced_hal_gpio_set_pin_output() in the seconds_app_timer_cb() function.
